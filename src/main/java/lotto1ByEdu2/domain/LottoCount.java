...
public class LottoCount {
	...
	// public LottoCount() {
	// 	this.lottoCount = 0; // 사용처에서 외부입력없으면, <관리 인스턴스변수 자리에  private = new LottoCount()의 기본생성자로 내가 초기화해줘야한다.>
	// }
	public LottoCount(Money money) {
	    this.lottoCount = money.toLottoCount();
	}
	//TODO wrap 4) 생성자는 여러개(파라미터 받을 갯수 정해줄 수 있음)일 수 도 있다.

	//TODO wrap 1): 파라미터 검증(필요시) + 파라미터 추가 및 수정(default name)
	public static LottoCount of(Money money) {
	...
	public void changeCount(Integer manualLottoCount) {
		this.lottoCount = this.lottoCount - manualLottoCount;
	}
	...
	// (1) implements Comparable<LottoCount> 이후
	// (2) compareTo를 오버라이딩 -> 파라미터를 동일한 객체(  LottoCount otherLottoCount  ) 로 수정
	// (3) return Integer.compare( this.lottoCount , otherLottoCount.getLottoCount());    으로  비교하게 하기s
