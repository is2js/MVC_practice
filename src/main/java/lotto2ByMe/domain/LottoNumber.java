...
public class LottoNumber implements Comparable<LottoNumber> {
	...
	// public LottoNumber() {
	// 	this.lottoNumber = 0; // 사용처에서 외부입력없으면, <관리 인스턴스변수 자리에  private = new LottoNumber()의 기본생성자로 내가 초기화해줘야한다.>
	// }
	...
	public int match(LottoNumber inputNumber) {
		// 일급vs일급 단위로 결과값1개가 최종완성 -> enum은 최종 결과값 [몇개당첨 - 상금] 매핑 예정..
		// -> 객vs객은 당첨인지 아닌지 위에 알려줘야함
		if (this.compareTo(inputNumber) == 0) {
			// if (this.lottoNumber.equals(inputNumber)) {
